...
Os dados cobrem **locais físicos cadastrados manualmente** na tabela `locations`. O sistema opera em **3 camadas complementares**:
...
Diferente de `location_metrics` (retém só 6 snapshots), os sinais têm **valor analítico acumulado**. Não há limpeza automática. Revisar após 30 dias de coleta para avaliar volume e adicionar particionamento por mês se necessário.
...
|---|---|---|
| `base_crowd_factor` | Fator estrutural do local (0–100, neutro=50) | `(fator - 50) × 0.3` pts |
...
O tempo de espera é calculado por interpolação linear dentro da faixa de score correspondente ao nível de fluxo:

| Flow | Faixa de score | Faixa de espera |
|---|---|---|
| `baixo` | 0 – 35 | 5 – 10 min |
| `médio` | 36 – 70 | 12 – 20 min |
| `alto` | 71 – 100 | 25 – 40 min |

#### Cálculo de tendência (`computeTrend`)

Compara a média da **primeira metade** dos últimos 6 snapshots com a média da **segunda metade**:

- Delta > +8 → `subindo`
- Delta < -8 → `caindo`
- Caso contrário → `estável`

Requer mínimo de 4 snapshots para calcular. Abaixo disso retorna `estável`.
...
3. Insere novo registro em `location_metrics`
4. Atualiza campos de estado em `locations`
...
7. App consulta `location_metrics` para enriquecer a lista com score e horário
...
|---|---|---|
| `locations` | Interna (Supabase PostgreSQL) | Cadastro manual de locais com atributos estruturais |
| `location_metrics` | Interna (Supabase PostgreSQL) | Histórico de snapshots calculados |
...
| Clima | **Pendente** | Campo `weather` reservado em `location_metrics`, ainda não populado |
...
Cadastro de locais. Estado atual do fluxo é mantido diretamente nesta tabela para leitura rápida sem JOIN.
...
| `base_crowd_factor` | `numeric` | Fator estrutural 0–100 (50 = neutro) |
...
| `trend` | `text` | Direção atual: `subindo`, `caindo`, `estável` |
...
**Sem limpeza automática** — dados acumulam para análise de padrões.
